...
export function activate(context: vscode.ExtensionContext) {
  ...
  const disposable = vscode.commands.registerCommand('codeAnalyzer.runAnalysis', async () => {
    ...
    const pythonPath = (config.get<string>('pythonPath') || 'python').trim();
    const defaultOutputFolder = (config.get<string>('defaultOutputFolder') || 'results').trim();
    ...
    const preferred = analyzerEntrypoints.find((u: vscode.Uri) =>
      u.fsPath.toLowerCase().includes(`${path.sep}code_analyzer${path.sep}`)
    );
    ...
    const outputFolderInput = await vscode.window.showInputBox({
      ...
      validateInput: (v: string) => (v.trim().length === 0 ? 'Output folder cannot be empty' : undefined)
    });
...
function generateAnalysisReadme(outputDir: string, analyzerRoot: string, targetPath: string): string {
  const files = [
    'domain_overview.md',
